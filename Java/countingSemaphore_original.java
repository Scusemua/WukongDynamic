      ...
      replyForRequestPermit ret;      
   	...
   	if (mode == REPLAY)
	      ...
	      // callerForReceiveEvent is not used here
			...
			ret = control.requestReceivePermitMS(((innerThread)Thread.currentThread()).getID(),getID(),semaphoreName + ":" + "P",getVersionNumber());  		
	...
	public void V() {
      ...
      replyForRequestPermit ret;
	      ...
	      // callerForReceiveEvent is not used here
			...
			ret = control.requestReceivePermitMS(((innerThread)Thread.currentThread()).getID(),getID(),semaphoreName + ":" + "V",getVersionNumber()); 
			callerForReceiveEvent = ret.getCallerForReceiveEvent();
			isChanged = ret.getIsChanged();
			...
			if (waitingP.size() > 0) { // this should always be true since P()'s have blocked
				...
				synchronized(oldest) {
				   if (mode==RT) {
	         		...
	         		// blocked P operations are never changed events (changed receive events are always completed)
						srEvent e = new srEvent(-2,getID(),-2,getVersionNumber(),
   						semaphoreName + ":" + "P",-1,UNACCEPTED_RECEIVE,
	         			getVectorTS(),"unacceptedReceive",false,nullOpenList,UNACCEPTED_RECEIVE,
	         			false); 
