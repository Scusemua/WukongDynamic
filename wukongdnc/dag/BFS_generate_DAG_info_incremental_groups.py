...
#rhc: clustering
                ...
                # Mark the groups of previous partition as completed. 
                # Do this one time, i.e., for first group, not for all the groups
                #
                # That is: for each previous_group, we need to compute its
                # edges in the DAG, i.e., identify the other groups to which it has
                # a fanout/fanin/faninNB/collapse. But this is not dependent
                # on group_name, the current group n the outer loop. So 
                # we only need to do this "for previous_group in groups_of_previous_partition"
                # one time, i.e., for the first group_name, we do not need to do
                # it for every group name.
                # 
                # Note also that above we are setting the state_info for group_name:
...
#rhc: clustering
                        ...
                        for receiverY in receiver_set_for_previous_group:
                            ...
                            if receiver_set_for_receiverY == None:
                                ...
                                # in every partition, so we will catch any sinks in this outer group_name loop. 
                                # 
                            ...
                            # number of groups that receive outputs from previous_group
                            ...
                            if length_of_sender_set_for_receiverY == 1:
                                ...
                                if length_of_receiver_set_for_previous_group == 1:
                                    ...
                                    # P only sends to one group receiverY, so collapse receiverY, i.e.,
                                    ...
                                    # effectively a task cluster at compile time.)
                                    ...
                                    # to group receiverY. (previous_group may do faninNBs
                                    # and fanouts, but we know receiverY is not a fanin or a faninNB
                                    # since it has only one input, so it must be a fanout.) 
...
#rhc: clustering
                                    # if we are doing runtime clustering, then we may want to 
                                    # know the number of nodes in group receiverY, so we track
                                    # for each fanout the size of the fanout task. This list
                                    # of sizes for each fanout is part of the DAG_info.
                                ...
                                # So we check all the tasks from which receiverY receives its inputs
                                # and if any of these tasks also outputs to some other task besides
                                # receiverY then we use a faninNB for receiverY.)
                                # 
                                isFaninNB = False
                                # Recall sender_set_for_receiverY is the groups that send outputs
                                # to receiverY, this could be one or more groups (since we know 
                                # that at the least previous_group sends output to receiverY)
                                #
                                ...
                                for senderZ in sender_set_for_receiverY:
                                    ...
                                    if len(receiver_set_for_senderZ) > 1:
                                        # if any group G sends output to reciverY and to any other group(s)
                                ...
                                if isFaninNB:
                                    ...
                                    # of faninNB receiverY. We will give the fanin/faninNB synch object 
                                    # created for receiverY its size so it knows how many inputs
                                    # to expect and it knows when all inputs have been received,
                                    # and for fanins it can determine which fanin was the last 
                                    # fanin and thus which executor/worker becomes the receiverY task.
                                    faninNBs.append(receiverY)
                                    faninNB_sizes.append(length_of_sender_set_for_receiverY)
                                else:
                                    # senderX sends its output only to receiverY, same for every other
                                    ...
                                    # of fanin receiverY. See comment about faninNB_sizes above.
                        ...
                        # this below. (Essentially, we make sure objects that could be 
                        # accessed concurrently are not shared, i.e., we access the object 
                        # O in one thread and a copy of the object O in the other 
                        # so there is no concurrent access of object O.)
...
# END
                        ...
                        state_info_of_previous_group.fanout_fanin_faninNB_collapse_groups_partitions_are_ToBeContinued = False
