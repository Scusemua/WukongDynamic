...
class User (object):
    ...
    def input_problem(problem : ProblemType):
        ...
        numbers = DivideandConquerTreeReduction.values
        ...
        if size != DivideandConquerTreeReduction.values.length:
    ...
    @staticmethod
    def output_result():
        result = FanInSychronizer.resultMap[DivideandConquerTreeReduction.root_problem_ID]
        ...
        _str = "Expected      :"
        for x in DivideandConquerTreeReduction.expected_order:
            _str = _str + str(x) + " "
        logger.debug(_str)
        ...
        error = False 
        for val in DivideandConquerTreeReduction.values:
            if result.numbers[i] != DivideandConquerTreeReduction.expected_order[i]:
                logger.debug("Error in expected value: result.numbers[{}] {} != expected_order[{}] {}".format(
                    i, result.numbers[i], i, DivideandConquerTreeReduction.expected_order[i]
                ))
                error = True 
        if not error:
            logger.debug("Verified.")

class DivideandConquerTreeReduction(object):
...
if __name__ == "__main__":
    logger.debug("Running DivideandConquerTreeReduction")
